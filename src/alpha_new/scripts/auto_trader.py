...
class TradingConfig:
    ...
    def load_from_config(cls):
        ...
        try:
            ...
            cls.BUY_PAYMENT_TYPE = system_config.get(
                "buy_payment_type", cls.BUY_PAYMENT_TYPE
            )
            cls.SELL_PAYMENT_TYPE = system_config.get(
                "sell_payment_type", cls.SELL_PAYMENT_TYPE
            )
            cls.TRADING_FEE_RATE = Decimal(
                str(system_config.get("trading_fee_rate", cls.TRADING_FEE_RATE))
            )
            cls.MIN_SELL_QUANTITY = Decimal(
                str(system_config.get("min_sell_quantity", cls.MIN_SELL_QUANTITY))
            )
            cls.PRICE_PRECISION = Decimal(
                str(system_config.get("price_precision", cls.PRICE_PRECISION))
            )
            cls.QUANTITY_PRECISION = Decimal(
                str(system_config.get("quantity_precision", cls.QUANTITY_PRECISION))
            )
            cls.ORDER_WAIT_TIMEOUT = system_config.get(
                "order_wait_timeout", cls.ORDER_WAIT_TIMEOUT
            )
            cls.WEBSOCKET_SETUP_DELAY = system_config.get(
                "websocket_setup_delay", cls.WEBSOCKET_SETUP_DELAY
            )
            cls.DEFAULT_MAX_RETRY = system_config.get(
                "max_retry", cls.DEFAULT_MAX_RETRY
            )

            cls.ENABLE_AUTO_SELL_ON_INSUFFICIENT_BALANCE = system_config.get(
                "enable_auto_sell_on_insufficient_balance",
                cls.ENABLE_AUTO_SELL_ON_INSUFFICIENT_BALANCE,
            )
            cls.MIN_BALANCE_THRESHOLD = Decimal(
                str(
                    system_config.get(
                        "min_balance_threshold", cls.MIN_BALANCE_THRESHOLD
                    )
                )
            )
...
class AutoTrader:
    def __init__(
        ...
        min_amount: float,
        buy_amount: float,
        target_total_amount: float | None = None,
    ...
    async def initialize_cumulative_amount(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"用户{self.user_id} {self.token_symbol}累计买入总额: {self.cumulative_buy_amount:.2f} USDT"
            )
            logger.info(
                f"用户{self.user_id} {self.token_symbol}目标买入总额: {self.target_total_amount:.2f} USDT"
            )
    ...
    async def run(
        self, stop_flag: asyncio.Event, buy_slippage: float, sell_slippage: float
    ):
        ...
        while not stop_flag.is_set() and (
            not self.target_total_amount or self.remaining_cycles > 0
        ):
            ...
            buy_price = (base_price * (Decimal("1") + buy_slip)).quantize(
                TradingConfig.PRICE_PRECISION, rounding=ROUND_DOWN
            )
            ...
            if buy_quantity < 1:
                ...
                continue
            try:
                actual_amount = (buy_price * buy_quantity).quantize(
                    Decimal("0.00000001"), rounding=ROUND_DOWN
                )
                payment_details = [
                    {
                        "amount": float(actual_amount),
                        "paymentWalletType": TradingConfig.BUY_PAYMENT_TYPE,
                    }
                ]
                order_data_logger.info(
                    json.dumps(
                        {
                ...
                order_data_logger.info(
                    f"用户{self.user_id} 买入订单API返回: {json.dumps(order, ensure_ascii=False)}"
                )
                if order.get("success") and order.get("data"):
                    ...
                    order_data_logger.info(
                        f"用户{self.user_id} 买入订单创建成功: {order_id}, 价格: {float(buy_price)}, 数量: {float(buy_quantity)}, 金额: {float(actual_amount)}"
                    )
                else:
                ...
                if result["error_type"] == ErrorType.INSUFFICIENT_BALANCE.value:
                    token_balance = await self.get_cached_token_balance(
                        self.token_symbol
                    )
                    if token_balance < TradingConfig.MIN_BALANCE_THRESHOLD:
                        logger.warning(
                            f"用户{self.user_id}买入失败：余额不足，{self.token_symbol}余额({token_balance})小于最小阈值({TradingConfig.MIN_BALANCE_THRESHOLD})，跳过卖出"
                        )
                        continue
                    if (
                        token_balance > 0
                        and TradingConfig.ENABLE_AUTO_SELL_ON_INSUFFICIENT_BALANCE
                    ):
                        logger.info(
                            f"用户{self.user_id}买入失败：余额不足，检测到{self.token_symbol}余额: {token_balance}，执行卖出"
                        )
                        await self.sell_all_balance(sell_slippage, int(token_balance))
                    else:
                        logger.warning(
                            f"用户{self.user_id}买入失败：余额不足，且无{self.token_symbol}代币或自动卖出已禁用"
                        )
                        continue
                elif result["action"] == "retry":
                    logger.warning(
                        f"用户{self.user_id}买入下单需要重试: {result['error_message']}"
                    )
                    await asyncio.sleep(result["retry_after"])
                    continue
                elif result["action"] == "wait":
                    logger.warning(
                        f"用户{self.user_id}买入下单需要等待: {result['error_message']}"
                    )
                    await asyncio.sleep(result["retry_after"])
                    continue
                else:
            ...
            try:
                ...
                status, filled_qty = await self.wait_order_filled_detail(
                    order_id, "BUY"
                )
                # 🔧 优雅退出：买入完成，更新状态
                if status == "FILLED":
                ...
                context = {"operation": "buy_wait", "order_id": order_id, "side": "BUY"}
                result = await self.error_handler.handle_error(e, self.user_id, context)
                logger.error(f"用户{self.user_id}买入等待异常: {e}")
                try:
                    await self.api.cancel_order(
                        order_id, self.token, TradingConfig.QUOTE_CURRENCY
                    )
                except Exception as cancel_error:
                    logger.warning(
                        f"用户{self.user_id}撤销订单失败（可能已成交）: {cancel_error}"
                    )
                continue
            if status == "FILLED":
                # 完全成交，等待0.3秒再卖出
                ...
                try:
                    ...
                    logger.warning(
                        f"用户{self.user_id}撤销部分成交订单失败（可能已成交）: {cancel_error}"
                    )
                logger.info(
                    f"用户{self.user_id}买入部分成交，撤单，卖出已成交部分: {filled_qty}"
                )
                await asyncio.sleep(0.3)
                await self.sell_all_balance(sell_slippage, filled_qty)
            else:
                ...
                try:
                    ...
                    logger.warning(
                        f"用户{self.user_id}撤销未成交订单失败: {cancel_error}"
                    )
                logger.info(f"用户{self.user_id}买入未成交，撤单，跳过本轮卖出")
                continue

    async def wait_order_filled_with_timeout(
        self, order_id: str, side: str, timeout: float
    ) -> tuple[str, float]:
        """等待订单完全成交，超时返回部分成交数量"""
        try:
    ...
    async def wait_order_filled_detail(
            ...
            if d.get("i") == order_id and d.get("S") == side:
                if d.get("X") == "FILLED":
                    ...
                    if side == "BUY":
                        ...
                        logger.info(
                            f"买入完全成交: 成交{filled_qty}, 手续费{commission}, 实际到账{actual_qty}"
                        )
                    else:
                        # 卖出：返回成交数量即可
                ...
                if d.get("X") == "PARTIALLY_FILLED":
                    ...
                    if side == "BUY":
                        actual_qty = filled_qty - commission
                        logger.info(
                            f"买入部分成交: 成交{filled_qty}, 手续费{commission}, 实际到账{actual_qty}"
                        )
                    else:
                        actual_qty = filled_qty
                ...
                if d.get("X") == "CANCELED":
                    ...
                    if side == "BUY" and filled_qty > 0:
                        actual_qty = filled_qty - commission
                        logger.info(
                            f"买入撤单: 已成交{filled_qty}, 手续费{commission}, 实际到账{actual_qty}"
                        )
                    else:
                        actual_qty = filled_qty
    ...
    async def wait_order_filled_detail_with_z(
        self, order_id: str, side: str
    ...
    async def sell_all_balance(self, sell_slippage: float, buy_filled_qty: float):
        ...
        float(TradingConfig.QUANTITY_PRECISION)
        ...
        logger.info(
            f"用户{self.user_id}实际余额: {actual_balance}, 预期卖出: {buy_filled_qty}"
        )
        ...
        if remain_qty < TradingConfig.MIN_SELL_QUANTITY:
            logger.warning(
                f"用户{self.user_id}可卖出数量({remain_qty})小于最小卖出量({TradingConfig.MIN_SELL_QUANTITY})，跳过卖出"
            )
            return
        ...
        while remain_qty >= TradingConfig.MIN_SELL_QUANTITY:
            ...
            is_last_retry = (
                remain_qty < TradingConfig.MIN_SELL_QUANTITY * 2
                or retry >= self.max_retry - 1
            )
            if is_last_retry:
                # 最后一次补单，查余额全仓卖出
                balance = await self.get_cached_token_balance(self.token_symbol)
                sell_quantity = Decimal(str(balance)).quantize(
                ...
                if sell_quantity < TradingConfig.MIN_SELL_QUANTITY:
                    order_data_logger.info(
                        f"用户{self.user_id} 最后一次补单余额({float(sell_quantity)})低于最小下单量，跳过"
                    )
                    break
                ...
                sell_price = (base_price * (Decimal("1") - sell_slip)).quantize(
                    TradingConfig.PRICE_PRECISION, rounding=ROUND_DOWN
                )

                payment_details = [
                    {
                        "amount": float(sell_quantity),
                        "paymentWalletType": TradingConfig.SELL_PAYMENT_TYPE,
                    }
                ]
                try:
                    order = await self.api.place_limit_order(
                    ...
                    order_data_logger.info(
                        f"用户{self.user_id} 最后一次补单全仓卖出: {json.dumps(order, ensure_ascii=False)}"
                    )
                except Exception as e:
                    order_data_logger.warning(
                        f"用户{self.user_id} 最后一次补单全仓卖出异常: {e}"
                    )
                break
            base_price = Decimal(str(await self.get_latest_price()))
            sell_slip = Decimal(str(sell_slippage))
            ...
            sell_quantity = Decimal(str(remain_qty)).quantize(
                TradingConfig.QUANTITY_PRECISION, rounding=ROUND_DOWN
            )
            ...
            try:
                ...
                order_data_logger.info(
                    f"用户{self.user_id} 卖出订单API返回: {json.dumps(order, ensure_ascii=False)}"
                )
                ...
                if not order.get("success") and order.get("code") == "481020":
                    ...
                    logger.warning(
                        f"用户{self.user_id}卖出余额不足(481020)，第{balance_error_count}次，卖出数量: {float(sell_quantity)}"
                    )

                    if balance_error_count >= max_balance_errors:
                        logger.error(
                            f"用户{self.user_id}连续{max_balance_errors}次余额不足，停止卖出避免死循环"
                        )
                        break

                    # 重新查询余额并调整卖出数量
                    current_balance = await self.get_cached_token_balance(
                        self.token_symbol
                    )
                    ...
                    if current_balance < TradingConfig.MIN_SELL_QUANTITY:
                        logger.warning(
                            f"用户{self.user_id}余额({current_balance})小于最小卖出量，停止卖出"
                        )
                        break
                ...
                if not order.get("success") and order.get("code") == "481013":
                    logger.warning(
                        f"用户{self.user_id}卖出金额过小(481013)，卖出数量: {float(sell_quantity)}, 金额: {float(sell_quantity * sell_price):.4f} USDT"
                    )
                    ...
                    logger.info(
                        f"用户{self.user_id}最小需要数量: {min_required_quantity:.4f}, 当前剩余: {float(remain_qty):.4f}"
                    )

                    if float(remain_qty) < min_required_quantity:
                        logger.warning(
                            f"用户{self.user_id}剩余数量不足以满足最小交易金额，停止卖出"
                        )
                        break
                ...
                if order.get("success") and order.get("data"):
                    order_id = order.get("data")
                    order_data_logger.info(
                        f"用户{self.user_id} 卖出订单创建成功: {order_id}, 价格: {float(sell_price)}, 数量: {float(sell_quantity)}, 金额: {float(sell_quantity * Decimal(str(sell_price)))}"
                    )
                    # 监听WebSocket订单状态
                    (
                        status,
                        filled_qty,
                        z_acc,
                    ) = await self.wait_order_filled_detail_with_z(order_id, "SELL")
                    if z_acc == 0:
                        # 没有任何成交，直接退出，避免死循环
                        break
                    ...
                    if status == "FILLED":
                        ...
                        if self.target_total_amount:
                            ...
                            logger.info(
                                f"用户{self.user_id}完成第{self.trade_count}次交易，剩余循环: {self.remaining_cycles}次"
                            )
                        else:
                            logger.info(
                                f"用户{self.user_id}完成第{self.trade_count}次交易"
                            )
                        await asyncio.sleep(0.3)
                        break
                ...
                if result["error_type"] == ErrorType.INSUFFICIENT_BALANCE.value:
                    logger.warning(
                        f"用户{self.user_id}卖出失败：{self.token_symbol}代币不足，当前余额: {float(remain_qty)}"
                    )
                    break
                if result["action"] == "retry":
                    logger.warning(
                        f"用户{self.user_id}卖出下单需要重试: {result['error_message']}"
                    )
                    await asyncio.sleep(result["retry_after"])
                    continue
                break
...
def get_alpha_id_from_token(
    token_symbol: str, token_info_path: str | None = None
) -> str:
    """根据代币符号获取alphaId"""
    import json
...
def get_price_stream_from_token(
    token_symbol: str, token_info_path: str | None = None
) -> str:
    import json
...
async def wait_for_current_trades_completion(
    traders: list, timeout: float = 30.0
) -> dict:
    """
    等待当前交易完成
    ...
    while time.time() - start_time < timeout:
        ...
        for trader, _, _ in traders:
            ...
            if state != "idle":
                all_idle = False
                logger.info(
                    f"用户{trader.user_id}: {state}, 订单ID: {trader.current_order_id}"
                )
    ...
    for trader, _, _ in traders:
        if trader.current_trading_state != "idle":
            ...
            logger.warning(
                f"用户{trader.user_id}交易未完成: {trader.current_trading_state}, 订单: {trader.current_order_id}"
            )
...
async def get_token_order_history_by_time_range(
    ...
    try:
        ...
        # 处理买入订单
        if buy_orders_response.get("code") == "000000" and buy_orders_response.get(
            "data"
        ):
        ...
        # 处理卖出订单
        if sell_orders_response.get("code") == "000000" and sell_orders_response.get(
            "data"
        ):
...
def load_trader_config(config_path: str | None = None) -> tuple[dict, dict]:
...
async def main():
    ...
    logger.info("=" * 60)
    logger.info("开始查询所有用户的累计买入总额（基于8点分界的时间范围）")
    logger.info("=" * 60)
    ...
    for user_id, api, headers, cookies, price_stream in users:
        ...
        sell_slippage = conf.get(
            "sell_slippage", global_conf.get("sell_slippage", 0.002)
        )
        target_total_amount = conf.get(
            "target_total_amount", global_conf.get("target_total_amount", None)
        )
    ...
    try:
        ...
        if graceful_shutdown:
            ...
            # 等待当前交易完成
            status_summary = await wait_for_current_trades_completion(
                traders, timeout=30.0
            )
        ...
        for error_type, stats in error_stats.items():
            if stats["count"] > 0:
                logger.info(
                    f"{error_type}: {stats['count']}次, 最后发生: {stats['time_since_last']:.1f}秒前"
                )
        ...
        try:
            await asyncio.wait_for(
                asyncio.gather(*ws_tasks, *trader_tasks, return_exceptions=True),
                timeout=5,
            )
        except TimeoutError:
