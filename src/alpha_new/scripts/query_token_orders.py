...
async def get_token_order_history_by_time_range(api: AlphaAPI, token_symbol: str) -> dict:
    ...
    try:
        ...
        # 查询卖出订单（支持多页合并）
        sell_orders = await fetch_all_orders(api, base_asset, "SELL", start_time, end_time)
...
async def main():
    ...
    try:
        ...
        # 查询订单历史统计
        order_stats = await get_token_order_history_by_time_range(first_api, token_symbol)
        
        if "error" not in order_stats:
            logger.info("=" * 60)
            logger.info("订单历史查询完成，详细统计:")
            logger.info(f"时间范围: {order_stats['time_range']['start']} ~ {order_stats['time_range']['end']}")
            logger.info(f"买入订单: {order_stats['buy_orders']['count']}笔")
            logger.info(f"买入总额: {order_stats['buy_orders']['total_amount']:.2f} USDT")
            logger.info(f"买入总量: {order_stats['buy_orders']['total_quantity']:.2f} {token_symbol}")
            logger.info(f"卖出订单: {order_stats['sell_orders']['count']}笔")
            logger.info(f"卖出总额: {order_stats['sell_orders']['total_amount']:.2f} USDT")
            logger.info(f"卖出总量: {order_stats['sell_orders']['total_quantity']:.2f} {token_symbol}")
            ...
            if order_stats['buy_orders']['orders']:
                ...
                for order in order_stats['buy_orders']['orders']:
                    logger.info(f"  订单ID: {order['orderId']}, 价格: {order['price']:.6f}, 数量: {order['quantity']:.2f}, 金额: {order['amount']:.2f}")
