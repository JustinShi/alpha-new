...
async def find_config_id(user_id: int, token_symbol: str, alpha_id: str = "") -> str | None:
    json_path = Path("data/airdrop_list.json")
    if json_path.exists():
        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
            for config in data.get("data", {}).get("configs", []):
                if (token_symbol and config.get("tokenSymbol") == token_symbol) or (alpha_id and config.get("alphaId") == alpha_id):
                    return config.get("configId")
    return None

async def query_config_id(api: AlphaAPI, token_symbol: str, alpha_id: str = "") -> str | None:
...
async def claim_for_user(user_id: int, token_symbol: str, alpha_id: str, target_dt: datetime, engine, retry_times: int = 3, retry_interval: int = 1, time_offset: float = 0.0) -> tuple[int, str]:
    ...
    async with async_session() as session:
        ...
        while True:
            ...
            if delta <= ADVANCE_MS / 1000:
                logger.info(f"[用户{user_id}] 触发领取时刻(本地校准): {datetime.fromtimestamp(now).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}，提前补偿: {ADVANCE_MS}ms")
                break
...
async def main() -> None:
    ...
    hour, minute, second = config.get("target_hour", 19), config.get("target_minute", 0), config.get("target_second", 0)
    ...
    valid_users = [uid for uid, score in zip(user_ids, scores) if score >= min_score]
    ...
    while True:
        ...
        if not claim_task_started and delta <= 30:
            logger.info(f"距离目标时间30秒，启动多用户并发监控...")
            claim_coros = [claim_for_user(uid, token_symbol, alpha_id, target_dt, engine, claim_retry_times, claim_retry_interval, time_offset) for uid in valid_users]
    ...
    for path in glob.glob(os.path.join(data_dir, "claim_log_*.json")):
        ...
        try:
            ...
            pass
    # 2. 删除所有旧的 claim_log_用户_时间.json 文件
    for path in glob.glob(os.path.join(data_dir, "claim_log_*_*.json")):
        os.remove(path)
    if claim_task:
