...
def clean(
    ...
    if not confirm:
        confirm = (
            Prompt.ask("确认删除这些文件?", choices=["y", "n"], default="n") == "y"
        )
    ...
    if confirm:
        ...
        console.print(
            Panel(
                f"[green]成功删除 {deleted_count} 个日志文件[/green]", title="清理完成"
            )
        )
    else:
...
def level(
    ...
    console.print(
        Panel(
            f"[green]模块 {module} 日志级别已设置为 {level.upper()}[/green]",
            title="日志级别设置",
        )
    )
...
def interactive():
    ...
    while True:
        ...
        choice = Prompt.ask(
            "请选择功能", choices=["1", "2", "3", "4", "0"], default="0"
        )
