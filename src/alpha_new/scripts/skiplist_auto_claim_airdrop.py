        ...
        try:
            ...
            for cfg in airdrop_list.get("data", {}).get("configs", []):
                if cfg.get("tokenSymbol") == token_symbol and (
                    cfg.get("claimInfo", {}).get("canClaim")
                    or cfg.get("claimInfo", {}).get("claimStatus") == "available"
                ):
...
async def main():
    ...
    if not config_id:
        ...
        return
    logger.info(
        f"检测到可领取空投({token_symbol})，configId={config_id}，所有用户开始领取..."
    )
        ...
        all_logs.append(
            {
                ...
                "user_id": res_uid,
                "result": msg,
                "script": "skiplist_auto_claim_airdrop",
            }
        )
    with open(log_path, "w", encoding="utf-8") as f:
