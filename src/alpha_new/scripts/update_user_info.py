...
async def main(user_id: int) -> None:
    ...
    async with async_session() as session:
        ...
        try:
            ...
            await update_user_info(session, user_id, user.name, user.email, user.score, login_status=login_status)
            return
        # 解析返回数据
        name = user_info["data"].get("firstName", "")
        email = user_info["data"].get("email", "")
        score = int(alpha_score["data"].get("sumScore", 0))
        await update_user_info(session, user_id, name, email, score, login_status=login_status)
        logger.info(f"用户{user_id}信息已更新: {name}, {email}, {score}, {login_status}")

if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        logger.error("用法: poetry run python -m alpha_new.scripts.update_user_info <user_id>")
    else:
